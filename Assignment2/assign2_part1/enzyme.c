...
//gcc enzyme.c smp2_tests.c testrunner.c -o enzyme -lpthread
...
void *run_enzyme(void *data) {
	...
	5. Create a while loop that only exits when please_quit is zero
	...
	*/
	...
	thread_info_t *new_data = (thread_info_t *) data;
	new_data->swapcount = 0;
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
	pthread_t currentId= pthread_self();
	if(new_data->string[0] == 'C'){
		pthread_cancel(currentId);
	}

	while(!please_quit){
		if(new_data->string[0] > new_data->string[1]){
			workperformed = 1;
			new_data->swapcount ++;
			char temp = new_data->string[0];
			new_data->string[0] = new_data->string[1];
			new_data->string[1] = temp;
		}
		if(use_yield){
			pthread_yield();
		}
