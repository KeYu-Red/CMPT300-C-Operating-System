...
sem_t sem_value;
int error = 0;
//gcc calc.c smp3_tests.c testrunner.c -o calc -lpthread
...
void *adder(void *arg)
{
    ...
    if(sem_wait(&sem_value)){
    	printErrorAndExit("Semaphore Fail\n");
    	return NULL;
    }
    if (timeToFinish()) {
		if(sem_post(&sem_value))
		printErrorAndExit("Cannot Post!\n");
	    return NULL;
	}
    ...
    if(pthread_mutex_lock(&mutex_lock)){
    	printErrorAndExit("Cannot Lock add!\n");
    }


	if (timeToFinish()) {
		if(pthread_mutex_unlock(&mutex_lock))
		printErrorAndExit("Cannot Unlock!\n");
	    return NULL;
	}
	*/
	...
	for (i = 0; i < bufferlen; i++) {
		...
		if(buffer[i]=='+'){
			startOffset = i;
			remainderOffset = i;
			while(startOffset > 0 && isNumeric(buffer[startOffset-1])){
				startOffset --;
			}
			while(remainderOffset+1 < bufferlen && isNumeric(buffer[remainderOffset+1])){
				remainderOffset++;
			}

			if(startOffset==i || remainderOffset==i){
				continue;
			}else{
				num_ops++;
				strncpy(v1,buffer+startOffset,i-startOffset);
				v1[i-startOffset]='\0';
				//puts(v1);
				value1 = string2int(v1);
				strncpy(v2,buffer+i+1,remainderOffset-i);
				v2[remainderOffset-i]='\0';
				//puts(v2);
				value2 = string2int(v2);
				add = value1+value2;
				int2string(add,tempt);
				strcpy(buffer_temp,buffer+remainderOffset+1);
...
void *multiplier(void *arg)
{
    ...
    /*if(pthread_mutex_lock(&mutex_lock)){
    	printErrorAndExit("Cannot Lock mul!\n");
    }
...
void *degrouper(void *arg)
{
    ...
    /*	
	if(pthread_mutex_lock(&mutex_lock)){
    	printErrorAndExit("Cannot Lock deg!\n");
    }
	if (timeToFinish()) {
		if(pthread_mutex_unlock(&mutex_lock))
		printErrorAndExit("Cannot Unlock!\n");
	...
	*/
	...
	for (i = 0; i < bufferlen; i++) {
	    ...
	    if(buffer[i]=='('){
			...
			//printf("********1**********\n");
			if(buffer[remainderOffset+1] != ')'){
			//	printf("********2**********\n");
				continue;
			}else{
			//	printf("********3**********\n");
				num_ops++;
				strncpy(v1,buffer+i+1,remainderOffset-i);
				v1[remainderOffset-i]='\0';
				//puts(v1);
				strcpy(buffer_temp,buffer+remainderOffset+2);
				buffer[i] = '\0';
				strcat(buffer,v1);
				strcat(buffer,buffer_temp);
				//puts(buffer);
...
int smp3_main(int argc, char **argv)
{
	...
	int res = sem_init(&sem_value, 0, 1);
	if (res != 0)
	 {
	  printErrorAndExit("Semaphore initialization failed");
	  return EXIT_FAILURE;
	 }
/*
    int ret = pthread_mutex_init(&mutex_lock, NULL);
    if (ret != 0) {
    	printErrorAndExit("mutex init failed\n");
        return -1;
    }*/
