...
void update_wait_time(thread_info_t *info) {
    ...
    info->wait_time += time_difference(&(info->suspend_time), &(info->resume_time));
}
...
void setup_sig_handlers() {
	...
	/* Setup timer handler for SIGUALRM signal in workers */
...
{
	wait_for_queue();
   	item_value.it_interval.tv_sec = QUANTUM;  
    item_value.it_interval.tv_nsec = 0;  
    item_value.it_value.tv_sec = QUANTUM;  
    item_value.it_value.tv_nsec = 0;  
	/* TODO: start the timer */
	timer_settime(timer, 0, &item_value, NULL);
